...
def get_category_metadata(cfg, metadata=None):
    ...
    if cfg.ANNOTATION.DATASET_NAME == 'EGO_ERROR':
        keysteps_dict = {
            'BlenderBananaPancakes': 14,
            'BreakfastBurritos': 11,
            'BroccoliStirFry': 25,
            'ButterCornCup': 12,
            'CapreseBruschetta': 11,
            'CheesePimiento': 11,
            'Coffee': 16,
            'CucumberRaita': 11,
            'DressedUpMeatballs': 16,
            'HerbOmeletWithFriedTomatoes': 15,
            'MicrowaveEggSandwich': 12,
            'MicrowaveFrenchToast': 11,
            'MicrowaveMugPizza': 14,
            'MugCake': 20,
            'PanFriedTofu': 19,
            'Pinwheels': 19,
            'Ramen': 15,
